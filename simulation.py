...
class Drone:
    ...
    def move_to_next_zone(self, next_zone):
        ...
        # print(self.graph['graph'][self.location])
        ...
        # resset capacity to -1
        # self.graph['graph'][self.location][connection_idx]["holde"] -= 1
        ...
        while not self.all_delivered():
            ...
            for  drone in self.track_drone:
                ...
                if drone.wait_turns > 0:
                    ...
                    move_track.append((drone.drones_id, drone.rest_zone_landing))
                    continue
                #  
                ...
                current_hold = zone_data.get("holde", 0)
                max_drones = zone_data.get("max_drones", 1)

                # skip full connection

                for idx, connection in enumerate(self.graph['graph'][drone.location]):
                    if connection['to'] == zone:
                        # print(connection['to'], zone, ": ", connection)
                        connection_idx = idx
                        capacity_link = connection["capacity"]
                        holde_link  = connection["holde"]
                        break
                # print("\tcapacity_link == holde_link: ",capacity_link ,  holde_link)
                ...
                self.graph['graph'][drone.location][connection_idx]["holde"] += 1  # <--------------------
                # print("\tcapacity_link == holde_link: ",capacity_link ,  self.graph['graph'][drone.location][connection_idx]["holde"])
                ...
                if zone_data.get('zone') == 'restricted':
                    ...
                    move_track.append((drone.drones_id, f"{leaving}-{landing}"))
